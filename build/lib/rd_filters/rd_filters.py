...
class RDFilters:
    def __init__(self, rules_file_name, save_all_rules=False):
        ...
        self.rule_list = []
        self.save_all_rules = save_all_rules
    ...
    def evaluate(self, lst_in):
        ...
        failed_rules = [] #list of all failed rules
        if mol is None:
        ...
        for row in self.rule_list:
            ...
            if len(mol.GetSubstructMatches(patt)) > max_val:
                if self.save_all_rules:
                    failed_rules.append(desc + " > %d" % (max_val))
                else: # just return first failed rule
                    return [smiles, name] + [desc + " > %d" % (max_val)] + desc_list #change here
        if len(failed_rules) == 0:
            return [smiles, name, "OK"] + desc_list #will work for both cases of save_all_rules
        else:
            all_failed_rules = ': '.join(failed_rules) #using ':' as a separator because other characters appear in the results
            return [smiles, name, all_failed_rules] + desc_list
